...
from HealthNet.static.reportlab.lib.colors import red, gray, lightgrey
from HealthNet.static.reportlab.lib.rl_accel import fp_str
from HealthNet.static.reportlab.lib.enums import TA_LEFT, TA_CENTER, TA_RIGHT, TA_JUSTIFY
from HealthNet.static.reportlab.lib.styles import _baseFontName
from HealthNet.static.reportlab.lib.utils import strTypes
from HealthNet.static.reportlab.pdfbase import pdfutils
from HealthNet.static.reportlab.pdfbase.pdfmetrics import stringWidth
from HealthNet.static.reportlab.rl_config import _FUZZ, overlapAttachedSpace, ignoreContainerActions, listWrapOnFakeWidth
import collections
...
class Image(Flowable):
    ...
    def __init__(self, filename, width=None, height=None, kind='direct',
        ...
        if not fp and os.path.splitext(filename)[1] in ['.jpg', '.JPG', '.jpeg', '.JPEG']:
            ...
            from HealthNet.static.reportlab.lib.utils import open_for_read
    ...
    def __getattr__(self,a):
        if a=='_img':
            from HealthNet.static.reportlab.lib.utils import ImageReader  #this may raise an error
...
class CondPageBreak(Spacer):
    ...
    def wrap(self, availWidth, availHeight):
        if availHeight<self.height:
            ...
            from HealthNet.static.reportlab.platypus.doctemplate import FrameBreak
...
def _listWrapOn(F,availWidth,canv,mergeSpace=1,obj=None,dims=None,fakeWidth=None):
    ...
    if cframe:
        from HealthNet.static.reportlab.platypus.doctemplate import _addGeneratedContent, Indenter
        doct_frame = cframe
    ...
    try:
        ...
        while F:
            ...
            if hasattr(f,'frameAction'):
                from HealthNet.static.reportlab.platypus.doctemplate import Indenter
...
def _flowableSublist(V):
    ...
    from HealthNet.static.reportlab.platypus.doctemplate import LCActionFlowable
    ...
    def split(self, aW, aH):
        ...
        if C0 or C1:
            if C0:
                ...
                from HealthNet.static.reportlab.platypus.doctemplate import NullActionFlowable
...
class _Container(_ContainerSpace):  #Abstract some common container like behaviour
    def drawOn(self, canv, x, y, _sW=0, scale=1.0, content=None, aW=None):
        ...
        from HealthNet.static.reportlab.platypus.doctemplate import ActionFlowable, Indenter
        x0 = x
        y0 = y
        pS = 0
...
class KeepInFrame(_Container,Flowable):
    ...
    def wrap(self,availWidth,availHeight):
        from HealthNet.static.reportlab.platypus.doctemplate import LayoutError
        mode = self.mode
...
class ImageAndFlowables(_Container,Flowable):
    ...
    def _findSplit(self,canv,availWidth,availHeight,mergeSpace=1,obj=None):
        ...
        for i,f in enumerate(F):
            ...
            H += h
            if H>availHeight:
                from HealthNet.static.reportlab.platypus.paragraph import Paragraph
                aH = availHeight-(H-h)
...
class FrameBG(AnchorFlowable):
    ...
    def __init__(self, color=None, left=0, right=0, start=True):
        ...
        self.start = start
        if start:
            from HealthNet.static.reportlab.platypus.doctemplate import _evalMeasurement
...
class FrameSplitter(NullDraw):
    ...
    def wrap(self,aW,aH):
        frame = self._frame
        from HealthNet.static.reportlab.platypus.doctemplate import NextPageTemplate,CurrentFrameFlowable,LayoutError
...
from HealthNet.static.reportlab.lib.sequencer import _type2formatter
_bulletNames = dict(
...
from HealthNet.static.reportlab.lib.styles import ListStyle
class ListItem:
    def __init__(self,
...
class DocPara(DocAssign):
    ...
    def wrap(self,aW,aH):
        ...
        P = self.klass
        if not P:
            from HealthNet.static.reportlab.platypus.paragraph import Paragraph as P
        style = self.style
        if not style:
            from HealthNet.static.reportlab.lib.styles import getSampleStyleSheet
