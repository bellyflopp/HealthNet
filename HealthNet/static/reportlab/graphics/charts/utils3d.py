...
from HealthNet.static.reportlab.graphics.shapes import Group, Drawing, Ellipse, Wedge, String, STATE_DEFAULTS, Polygon, Line

def _getShaded(col,shd=None,shading=0.1):
    if shd is None:
        from HealthNet.static.reportlab.lib.colors import Blacker
        if col: shd = Blacker(col,1-shading)
    return shd

def _getLit(col,shd=None,lighting=0.1):
    if shd is None:
        from HealthNet.static.reportlab.lib.colors import Whiter
...
if __name__=='__main__':
    ...
    from HealthNet.static.reportlab.lib.colors import lightgrey, pink
    D = Drawing(300,200)
