...
import sys, codecs
from HealthNet.static.reportlab.lib.utils import isSeq, isBytes, isStr, isPy3
...
class _isListOfShapes(Validator):
    ...
    def test(self, x):
        from HealthNet.static.reportlab.graphics.shapes import Shape
        if isSeq(x):
            answer = 1
            for e in x:
...
from HealthNet.static.reportlab.lib.normalDate import NormalDate
...
class _isValidChild(Validator):
    ...
    def test(self, x):
        ...
        from HealthNet.static.reportlab.graphics.shapes import UserNode, Shape
