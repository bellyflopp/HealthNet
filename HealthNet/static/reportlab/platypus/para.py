...
class paragraphEngine:
    ...
    def __init__(self, program = None):
        from HealthNet.static.reportlab.lib.colors import black
        if program is None:
            program = []
        ...
        from HealthNet.static.reportlab.lib.enums import TA_LEFT
...
class Para(Flowable):
    ...
    def compileProgram(self, parsedText, program=None):
        ...
        a( ("face", fn ) )
        from HealthNet.static.reportlab.lib.fonts import ps2tt
...
##        if dump: stop
    ...
    def shiftfont(self, program, face=None, bold=None, italic=None):
        ...
        self.bold = bold
        self.italic = italic
        from HealthNet.static.reportlab.lib.fonts import tt2ps
...
class SeqDefaultObject(NameObject):
    ...
    def getOp(self, tuple, engine):
        from HealthNet.static.reportlab.lib.sequencer import getSequencer
        globalsequencer = getSequencer()
        attr = self.attdict
        try:
            default = attr['id']
...
def handleSpecialCharacters(engine, text, program=None):
    from HealthNet.static.reportlab.platypus.paraparser import greeks
...
def Paragraph(text, style, bulletText=None, frags=None, context=None):
    ...
    if "&" not in text and "<" not in text:
        ...
        from HealthNet.static.reportlab.lib import rparsexml
...
def test2(canv,testpara):
    ...
    from HealthNet.static.reportlab.lib.units import inch
    from HealthNet.static.reportlab.lib.styles import ParagraphStyle
    from HealthNet.static.reportlab.lib import rparsexml
...
def test():
    ...
    #print test_program; return
    from HealthNet.static.reportlab.pdfgen import canvas
    from HealthNet.static.reportlab.lib.units import inch
    fn = "paratest0.pdf"
    c = canvas.Canvas(fn)
